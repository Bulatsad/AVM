...
namespace AVI
{
    class MUL : IAsseblerVirtualModule
    {
        ...
        {
            return baitCode == EBaitCodeList["mulr"] ||
                baitCode == EBaitCodeList["mulm"];
        }
        ...
        public void Execute(ref byte[] Registers, ref byte[] RAM)
        {
            int ip = RM.ReadReg("ip", Registers);
            byte code = RAM[ip];
            if (code == EBaitCodeList["mulr"]) EMULR(ref Registers, ref RAM);
            else if (code == EBaitCodeList["mulm"]) EMULM(ref Registers, ref RAM);
        }
